...
| 1 | [Анализ рынка игр](https://github.com/StasKolomin/yandex-practicum/blob/main/1_module_proj/1_game_analis.ipynb) | **Сборный проект** 1го модуля. Нужно выявить определяющие успешность игры закономерности. Это позволит сделать ставку на потенциально популярный продукт и спланировать рекламные кампании. | Тщательная предобработка (пропуски, некорректные значения) | pandas, numpy, scipy, matplotlib, seaborn, plotly|
| 2_1 | | **"Введение в машинное обучение"** | | |
